...
export default function CategoryMovies({
  ...
  // Use initialData directly for the initial state
  ...
  const [selectedCategorySlug, setSelectedCategorySlug] = useState(initialSlug);

  const [filters, setFilters] = useState(() => {
    const params = initialData?.params || {};
    return {
      quoc_gia: params.filterCountry?.[0] || "",
      the_loai: params.filterCategory?.[0] || "", // Sub-category
      ngon_ngu: params.filterType?.[0] || "",
      nam_phat_hanh: params.filterYear?.[0] || "",
      sort_field: params.sortField || "_id",
  ...
  // Options for filters (Country, Category, Language, Release Year, Sort)
  ...
  const buildApiUrl = useCallback(
    (pageToFetch = 1) => {
      const url = new URL(`${baseUrl}/v1/api/the-loai/${selectedCategorySlug}`);
    ...
    [filters, baseUrl, selectedCategorySlug, initialLimit]
  );
  ...
  const fetchMovies = useCallback(
    async (pageToFetch = 1) => {
      ...
      try {
        ...
        const newUrl = new URL(
          `${window.location.origin}/the-loai/${selectedCategorySlug}`
        );
        newUrl.searchParams.set("page", pageToFetch.toString());
        newUrl.searchParams.set("limit", initialLimit.toString());

        // Conditionally add/remove search parameters to the URL
        if (filters.sort_field) {
    ...
    [buildApiUrl, filters, initialLimit, selectedCategorySlug]
  );

  // Modified handleFilterChange to accept name and value directly for consistency with button clicks
  ...
  const handleApplyFilters = () => {
    fetchMovies(1); // Fetch from page 1 when filters are applied
  };
  ...
  // Effect to handle changes in initialSlug (e.g., navigating from /the-loai/action to /the-loai/comedy)
  useEffect(() => {
    // Only update states if the initialSlug has actually changed
    if (initialSlug !== selectedCategorySlug) {
      setSelectedCategorySlug(initialSlug);
      // Reset filters to default when category changes via URL slug
      setFilters({
        quoc_gia: "",
        the_loai: "", // This is for sub-categories
        ngon_ngu: "",
        nam_phat_hanh: "",
        sort_field: "_id",
        sort_type: "desc",
      });
      // Immediately fetch data for the new category slug
      fetchMovies(1);
    }
  }, [initialSlug, selectedCategorySlug, fetchMovies]);
      ...
      <div className="">
        ...
        {isFilterExpanded && (
          <div className="mt-4 flex flex-col gap-4 text-xs text-white border border-gray-200/20 rounded-lg p-4">
            {/* Quốc gia Filter - Updated to buttons */}
            <div>
            ...
            {/* Thể loại Filter (Sub-categories within the main category) - Updated to buttons */}
            <div>
            ...
            {/* Ngôn ngữ Filter - Updated to buttons */}
            <div>
            ...
            {/* Năm phát hành Filter - Updated to buttons */}
            <div>
            ...
            {/* Sắp xếp Filter - Updated to buttons */}
            <div>
            ...
            <div className="flex justify-start gap-2 mt-4">
              <button
                onClick={handleApplyFilters} // Nút Tìm Kiếm
