...
export default function CountryMovies({
  ...
  const [selectedCountrySlug, setSelectedCountrySlug] = useState(initialSlug);

  const [filters, setFilters] = useState(() => {
    const params = initialData?.params || {};
    return {
      quoc_gia: params.filterCountry?.[0] || "",
      the_loai: params.filterCategory?.[0] || "",
  ...
  const countryOptions = [
    ...
    { label: "UAE", value: "ae" },
    { label: "Ả Rập Xê Út", value: "sa" },
    { label: "Thổ Nhĩ Kỳ", value: "tr" },
    { label: "Brazil", value: "br" },
    { label: "Nam Phi", value: "za" },
    { label: "Na Uy", value: "no" },
    { label: "Châu Phi", value: "xx" },
    { label: "Quốc Gia Khác", value: "zz" },
  ];
  ...
  const buildApiUrl = useCallback(
    (pageToFetch = 1) => {
      const url = new URL(`${baseUrl}/v1/api/quoc-gia/${selectedCountrySlug}`);
      ...
      if (filters.quoc_gia && filters.quoc_gia !== selectedCountrySlug) {
        url.searchParams.set("country", filters.quoc_gia);
      } else if (filters.quoc_gia === "") {
    ...
    [filters, baseUrl, selectedCountrySlug, initialLimit]
  );
  ...
  const fetchMovies = useCallback(
    async (pageToFetch = 1) => {
      ...
      try {
        ...
        const newUrl = new URL(
          `${window.location.origin}/quoc-gia/${selectedCountrySlug}`
        );
    ...
    [buildApiUrl, filters, initialLimit, selectedCountrySlug]
  );
  ...
  useEffect(() => {
    if (initialSlug !== selectedCountrySlug) {
      setSelectedCountrySlug(initialSlug);
      setFilters({
        quoc_gia: initialSlug,
        the_loai: "",
        ngon_ngu: "",
        nam_phat_hanh: "",
        sort_field: "_id",
  ...
  }, [initialSlug, selectedCountrySlug, fetchMovies]);
